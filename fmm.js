...
let cookie_ongkaijin777 = "remember_web_59ba36addc2b2f9401580f014c7f58ea4e30989d=%3D";
...
let nodes = JSON.parse(await fs.readFile('../getEpisodes 13221 25-05-2026.json', 'utf-8'));
let priority = [];
let count = [];
...
async function loop (start) {
	...
	while (i < count.length) {
		let page = await fetch("https://fmfenxiang.com/sound/" + count[i].id, {headers: {cookie: cookie_ongkaijin777}});
		...
		if (!page.status === 200) {
			...
			let player = JSON.parse('"' + doc.window.document.querySelector("#fm-player-col").getAttribute("x-init") + '"').match(/\(([^)]+)\)/)[1].split(',').map((i) => i.trim().replace(/^['"]|['"]$/g, ''));
			let url = player[0];
			...
			try {
				...
				await fs.mkdir ("../fmfenxiang-log/" + count[i].id, {recursive: true});
				await fs.writeFile("../fmfenxiang-log/" + count[i].id + "/" + "dir.txt", filename, 'utf-8');
				await fs.writeFile("../fmfenxiang-log/" + count[i].id + "/" + filename + ".txt", page.status + " " + resp.status + " " + Date.now() + " " + url + " " + new Uint8Array(await crypto.subtle.digest('SHA-1', await resp2.arrayBuffer())).toHex() + " " + "cookie_ongkaijin777", 'utf-8');
				await checkpoint(i);
			} catch (error) {
				console.log(error);
				return loop(i + 1);
